...
void Mount_SD (const TCHAR* path)
{
	...
	if (fresult != FR_OK) WriteDebug ("ERR Mount SD");
	else WriteDebug("SD Mounted");
}

void Unmount_SD (const TCHAR* path)
{
	fresult = f_mount(NULL, path, 1);
	if (fresult == FR_OK) WriteDebug ("SD UNMOUNTED");
	else WriteDebug("ERR UNMOUNT SD");
}
